...
| H8 | Multiple coding-agents driving multiple robots simultaneously is the right new territory worth pioneering; sticking to single-agent harness loops is not. **Note: CooperBench (arXiv:2601.13295) provides the first quantified counter-evidence on the coding-agent axis — multi-agent coding collaboration shows 30% lower success rates than agents working individually. H8 design must acknowledge this baseline failure mode.** | Vector 1 (harness upstream); Vector 6 (multi-agent embodied AI) |
...
- **Harness-as-service primitives**: Anthropic Managed Agents 2026-05 bundle — `memory` (filesystem-backed cross-session persistence, public beta 2026-04-23), `dreaming` (between-session memory refinement, research preview 2026-05-06), `multiagent orchestration` (up to 20 parallel specialists in one session, public beta 2026-05-06), `outcomes` (rubric-driven grader running in independent Claude instance, public beta 2026-05-06), `self-hosted sandboxes` (public beta), `MCP tunnels` for private-network MCP servers (research preview). This is the first time harness-as-service ships a coherent multi-agent SDK with persistent memory and rubric grading; materially lowers H8 implementation cost.
...
- **Governance signals**: foundation transitions, CVE bulletins, trademark disputes, founder moves (Steinberger → OpenAI), Anthropic-OpenClaw relationship status; NVIDIA NemoClaw (enterprise wrap, Apache 2.0 early preview from GTC 2026-03-16 — NVIDIA OpenShell sandbox + Privacy Router + Nemotron local models; does not threaten Mode 2, adds an audit/security layer above OpenClaw)

**Expected changes**:

- OpenClaw foundation governance maturity (or stall) in 2026
- Whether Hermes / Nanobot / OpenHarness coalesce into a coherent "post-OpenClaw Python line" or stay fragmented single-user agents. As of 2026-05 the trend is **toward coalescing** (active maintenance across multiple variants, named `OpenClaw-migrate` semantics emerging in more than one variant), but no single winner has emerged yet — track at the category level rather than per-entity.
...
- **Watchlist (not investing yet)**: ManiSkill3 (HF team, GPU-parallel + PettingZoo multi-agent + room-scale); Habitat 3.0 + PARTNR (Meta, most mature multi-agent LLM evaluation benchmark); Isaac Sim + Isaac Lab (NVIDIA — Isaac Lab 3.0 Beta on develop branch as of 2026-05, ground-up rewrite, Newton/PhysX multi-backend, kit-less install); Isaac Lab-Arena (alpha, composable task curation + GR00T/OpenPi evaluation on Isaac Lab 3.0); Genesis (pre-1.0, performance claims unverified); RoboCasa; mujoco_playground; mujoco-mjx; **mujoco-warp (officially released 2026-05, no longer alpha)**; **mjlab (Isaac-Lab-style API + MuJoCo Warp physics — opens an Isaac-Lab path on MuJoCo backend that did not exist when this card was first written; matters as an H2 option-space expansion if MolmoSpaces multi-agent never materializes)**
...
- **Multi-coding-agent precedent in code (not embodied)**: Cursor recursive Planner/Worker/Judge (architectural ancestor of multi-agent harness); **CooperBench (arXiv:2601.13295, 2026-01) — 652-task two-agent collaborative coding benchmark; quantified counter-evidence to multi-agent harness design: "agents achieve on average 30% lower success rates when working together compared to performing both tasks individually, across the full spectrum of task difficulties." Any H8 route design must acknowledge this.**
- **H8-axis precedents (each axis covered, none yet combining both)**: **CaP-X (arXiv:2603.22435, 2026-03) — single coding-agent + single simulated robot in code-as-action form, closest match for the "coding agent drives simulated robot" axis**; **LLM-Foraging (arXiv:2605.01461, 2026-05) — multi-LLM decision-maker + multi-robot foraging, but LLM outputs discrete tactical waypoint selection, NOT code-as-action**. Together they bracket H8's missing center: neither combines code-as-action with multi-embodiment in a shared task.
- **Watchlist (the gap that makes H8 risky)**: any paper, blog, or repo describing **code-as-action coding agents** simultaneously driving **multiple** simulated robots in a **shared task**. Tactical-decision-only or planner-only LLM routes do not qualify (LLM-Foraging would not have fired this watch entry under the tightened language); single-robot CaP work does not qualify either. Until something at this intersection appears, H8's "no public precedent" remains true and roboclaws is at the frontier — but with the two axes now covered separately the frontier window is shrinking.
...
- **Multi-agent-multi-robot precedent appears**: any paper, blog, or repo demonstrating **code-as-action coding agents** simultaneously driving **multiple** simulated robots in a **shared task** (affects H8 directly — currently "no public precedent" is the load-bearing claim; tighter than the original 2026-05 language to prevent false positives from tactical-decision-only multi-LLM work like LLM-Foraging, which would otherwise trigger without matching the H8 architectural bet)
...
- **Harness-as-service SDK lowers multi-agent harness implementation cost**: any cloud harness provider ships a feature that materially shortens what roboclaws would otherwise have to build (parallel-specialists primitive, rubric-based grader, persistent cross-agent memory, sandbox-aware multi-agent execution). Already fired 2026-05-06 with Managed Agents `multiagent orchestration` + `outcomes`; formalizing the trigger captures the lesson that SDK-level pacing events directly affect H4 and H8.
...
- 2026-05-23 (post-checkpoint architecture revisions): eight architecture-level updates derived from the 2026-05 checkpoint (`roboclaws/docs/research-checkpoints/2026-05.md` §5). Implementation-detail micro-adjustments (specific PyPI versions, fork attribution, blacklist domain additions, sub-version updates of Cursor / Composer / Automations) deliberately rejected per the "architecture not implementation" principle.
  1. V1 entity list: added the Anthropic Managed Agents 2026-05 bundle (memory + dreaming + multiagent orchestration + outcomes + self-hosted sandboxes + MCP tunnels) as a single named entity, because this is the first time harness-as-service ships a coherent multi-agent SDK with persistent memory and rubric grading and materially lowers H8 implementation cost.
  2. V2 entity list: added NVIDIA NemoClaw (GTC 2026-03-16 early preview, Apache 2.0) to governance signals as an enterprise wrap above OpenClaw — does not threaten Mode 2, adds an audit/security layer.
  3. V2 expected change: noted that the Python-variant-coalescing trend is now "leaning yes" but no single winner has emerged; tracked at category level rather than per-entity.
  4. V3 entity list: marked MuJoCo Warp as officially released (no longer alpha); added mjlab as a new entry because it opens an Isaac-Lab-API-on-MuJoCo path that did not exist when this card was first written, expanding H2's option space if MolmoSpaces multi-agent never materializes; mentioned Isaac Lab 3.0 Beta and Isaac Lab-Arena (alpha) as substrate-layer events.
  5. V6 entity list: added CooperBench (arXiv:2601.13295) with explicit citation of the 30% finding; added CaP-X (arXiv:2603.22435) and LLM-Foraging (arXiv:2605.01461) as the two H8-axis precedents that bracket H8's missing center.
  6. H8 hidden-assumption description: cited CooperBench's 30% finding directly in the table row, because any H8 route design must acknowledge multi-agent collaboration's quantified failure mode.
  7. §Triggers: tightened the multi-agent-multi-robot precedent trigger to require code-as-action + multi-embodiment combination, preventing false positives from tactical-decision-only multi-LLM work.
  8. §Triggers: added a new "Harness-as-service SDK lowers multi-agent harness implementation cost" trigger, formalizing the lesson from the Managed Agents 2026-05-06 event.
- This card formalizes a vectors structure for a repo that had already produced one large ad-hoc checkpoint. The first checkpoint written *under* this card (2026-05) produced the architecture-level revisions above, so the card has at least one concrete validation pass. Continue checking in future checkpoints that: (1) all 7 vectors produce useful, distinct output; (2) the 8 hidden assumptions each actually get challenged; (3) the period delta remains visible across OpenClaw governance, MolmoSpaces multi-agent, Nav2 pilot status, and multi-agent-multi-robot precedent.
- If two consecutive checkpoints leave all assumptions unchallenged, the vectors design has blind spots and needs revision. Entity lists are seeded from the 2026-04 checkpoint's Appendix A.1–A.12 plus current architecture state; future checkpoints should continue surfacing and adding entities missed at card-writing time.
